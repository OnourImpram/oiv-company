...
def home(lang):
 ...
 compass=f'''<div class="compass" role="group" aria-label="{d['compassLabel']}"><svg class="compass-lines" viewBox="0 0 500 440" fill="none" aria-hidden="true"><circle cx="250" cy="220" r="181"/><circle cx="250" cy="220" r="116"/><circle cx="250" cy="220" r="90"/><path d="M250 18v405M49 220h400M93 66l314 308M90 378 408 60" class="faint"/><circle class="point" cx="250" cy="39" r="3"/><circle class="point" cx="250" cy="104" r="3"/><circle class="point" cx="250" cy="336" r="3"/><circle class="point" cx="160" cy="220" r="3"/><circle class="point" cx="340" cy="220" r="3"/></svg><div class="compass-center"><img src="{{{{root}}}}assets/img/oiv-logo.png" alt="" width="480" height="271"></div>{nodes}<p class="compass-aside">{'<br>'.join(d['compassAside'])}</p><p class="compass-quote">“{d['compassQuote']}”</p></div>'''
 projects=''.join(f'''<article class="featured-project"><a class="project-mark" href="{link(lang,'work','#'+pid)}"><img src="{{{{root}}}}assets/img/{pid}.webp" alt="{name}" width="240" height="240" loading="lazy"></a><div><p class="project-role">{d[pid+'Role']}</p><h3><a href="{link(lang,'work','#'+pid)}">{name}</a></h3><p>{d[pid+'Copy']}</p>{textlink(('Çalışmayı inceleyin' if lang=='tr' else 'Explore the project'),link(lang,'work','#'+pid))}</div></article>''' for pid,name in [('mergen','Mergen'),('mneme','mneme')])
...
<section class="featured dark section" id="work" data-od-id="work"><div class="container featured-grid"><div class="featured-heading"><h2>{d['workTitle']}</h2>{textlink(d['viewProjects'],link(lang,'work'))}</div>{projects}</div></section>
